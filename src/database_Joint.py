...
from mechanism_components import Joint
class Joint_db(Joint):
    
    db_connector = DatabaseConnector().get_table("joint")
    
    def __init__(self,index:str, name : str, x: int, y :int, is_fixed : bool):
        super().__init__(index,name,x,y,is_fixed)
    
    @classmethod
    def instantiate_from_dict(cls, data: dict) -> Self:
        return cls(data['index'], data['name'], data['x'], data['y'], data['is_fixed'])
    
    def get_index(self) -> str:
        return F"{self.index}"
    
    def __str__(self) -> str:
        return F"index {self.index} Joint: {self.name} with coordinates {self.x},{self.y} and with the extra parameter {self.is_fixed}"
